...
class Utils
{
    public static int[, ,] Generate3DLabyrinth(int cubeSize)
    {
        ...
        SaveLabyrinthStructure(theCube);
    ...
    public static bool DisplayChallenge(List<Challenge> questions)
    {
        ...
        while (true)
        {
            ...
            if (key.Key == ConsoleKey.D1 || key.Key == ConsoleKey.D2 || key.Key == ConsoleKey.D3 ||
                key.Key == ConsoleKey.NumPad1 || key.Key == ConsoleKey.NumPad1 || key.Key == ConsoleKey.NumPad1)
            {
                break;
            }
            else
    ...
    public static Category ChooseCategory()
    {
        ...
        do
        {
            ...
            {
                Console.ForegroundColor = ConsoleColor.Red;
                Console.WriteLine("No Category under this number");
                Console.ForegroundColor = ConsoleColor.Gray;
            }
    ...
    public static void WriteToCSV(List<Score> scores)
    {
        ...
        {
            string line = string.Format("{0,-20}:{1}\n", scores[i].PlayerName, scores[i].PlayerScore);
